...
export class PoolFilters {
  ...
  constructor(
    ...
    */
    ...
    if (CHECK_META_WORDS) {
      this.filters.push(new MetaWordsFilter(connection, getMetadataAccountDataSerializer(), WORDS_LIST));
    }
    

    if (CHECK_IF_MUTABLE) {
      this.filters.push(new MutableFilter(connection, getMetadataAccountDataSerializer(), CHECK_IF_MUTABLE));
    }
